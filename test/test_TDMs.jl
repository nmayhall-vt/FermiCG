...
@testset "TDMs" begin
    ...
    cluster_ops_local = Vector{FermiCG.ClusterOps}()
    for ci in clusters
        push!(cluster_ops, FermiCG.ClusterOps(ci,3)) 
        push!(cluster_ops_local, FermiCG.ClusterOps(ci,2)) 
    end
    ...
    ref = [  0.997916  0.064526
           -0.064526  0.997916]
    test =cluster_ops[2]["AA"][((2, 1), (0, 1))][2,:,:] 
    ...
    test =cluster_ops[1]["aa"][((2, 1), (4, 1))][2,:,:] 
    @test isapprox(abs.(ref), abs.(test), atol=1e-5)
    ...
    test =cluster_ops[2]["Aa"][((1, 1), (1, 1))][3,:,:] 
    @test isapprox(abs.(ref), abs.(test), atol=1e-5)
    ...
    test =cluster_ops[1]["Bb"][((0, 2), (0, 2))][5,:,:] 
    @test isapprox(abs.(ref), abs.(test), atol=1e-5)
    ...
    test =cluster_ops[1]["Ab"][((2, 3), (1, 4))][5,:,:] 
    @test isapprox(abs.(ref), abs.(test), atol=1e-5)
