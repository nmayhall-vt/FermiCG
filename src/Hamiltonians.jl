...
Return energy defined by `rdm1` and `rdm2`
"""
function compute_energy(ints::InCoreInts, rdm1a, rdm1b, rdm2aa, rdm2ab, rdm2bb)
    e = ints.h0
    e += sum(ints.h1 .* rdm1a) + sum(ints.h1 .* rdm1b)
    e += .5*sum(ints.h2 .* rdm2aa)
    e += .5*sum(ints.h2 .* rdm2bb)
    e += .5*sum(ints.h2 .* rdm2ab)
    e += .5*sum(ints.h2 .* permutedims(rdm2ab,(3,4,1,2)))
    # @tensor begin
    # 	e  += .5 * (ints.h2[p,q,r,s] * rdm2[p,q,r,s])
    # end
    return e
